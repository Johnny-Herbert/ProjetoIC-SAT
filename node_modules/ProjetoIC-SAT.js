...
exports.solve = function (fileName) {
  ...
  function readClauses(text)
  {
    ...
    let achou = false
    //variavel que indica a posição onde vou armazenar no meu array auxiliar
    let posicao = 0
    let array = [] 
    let clauses = []
    //Transformo cada linha em um array dando split no espaço
    ...
    for(i = 0; i < linhas.length; i++)
    {
        for(j = 0; j < linhas[i].length; j++)
        {
          //verifico se eu eu achei o cnf
            ...
            if(achou)
            {
                ...
                {
                    //só zera a posição quando chegar no fim da causula(com valor zero) pois pode ter uma clausula que esteja em duas linhas
                    posicao = 0
                }
        ...
        if(achou && posicao == 0 && array.length != 0)
        {
            clauses.push(array)
            array = []
        }
